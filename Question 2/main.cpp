    ...
    if (file.is_open()) 
    {
        while (getline(file, line))
        {
            if (line.find("author") != string::npos) 
            {
                ...
                replace(authorList.begin(), authorList.end(), ',', ' ');  // Convert commas to spaces
...
void printMapContents(const map<string, vector<BibEntry>>& authorMap) 
{
    for (const auto& entry : authorMap) {
        cout << "Author: " << entry.first << endl;
...
int main() {
    ...
    cout << "Printing all authors and their publication details:" << endl;
    printMapContents(authorMap);
